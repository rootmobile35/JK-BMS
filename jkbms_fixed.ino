...
FirebaseData fbdo;
// FIX: separate FirebaseData object for the settings stream - the library
// requires a dedicated object per concurrent stream/request, sharing `fbdo`
// between the upload calls and a stream causes them to stomp on each other.
...
// ---------------------------------------------------------------------------
// FIX: real unlock/authentication frame. Uses the SAME 20-byte frame layout
// as writeRegister() (AA 55 90 EB <cmd> <len> <payload...> <checksum>),
// which is the layout your own read-side parsing confirms is correct - the
// old password attempts used a completely different, wrong layout.
//
// UNCERTAIN PIECE, please verify: the command byte (frame[4] = 0x00) for
// "unlock/authenticate" is my best-available reconstruction, not from an
// official spec (JK doesn't publish one). If writes still get ignored after
// this, the most reliable way to get the exact byte is to sniff the real JK
// BMS phone app's BLE traffic once (Android: enable "Bluetooth HCI snoop
// log" in Developer Options, use the JK BMS app to change one setting,
// pull the log via `adb bugreport` or Settings > System > Developer options
// > Bluetooth HCI log, then open it in Wireshark and look at the write to
// characteristic ffe1 right after you enter the password). That trace will
// give you the byte-perfect unlock frame instead of a guess.
// ---------------------------------------------------------------------------
bool JKBMS::unlockBMS() {
  ...
  frame[2] = 0x90;
  frame[3] = 0xEB;
  frame[4] = 0x00;   // Command: unlock/authenticate - VERIFY (see comment above)
  ...
  authenticated = ok;
  lastAuthTime = millis();
  delay(300); // let the BMS process the unlock before the next frame goes out
  return ok;
}
...
// FIX: NEW - Firebase stream callback. Fires whenever the dashboard writes
// to .../settings/<key> (single-leaf writes, which is what saveSetting()
// does) or on the initial stream connect (whole-object dump).
void firebaseSettingsStreamCallback(FirebaseStream data) {
  String path = data.dataPath(); // e.g. "/cellOvp", or "/" on initial sync
  ...
  if (path.length() > 1 && path.startsWith("/")) {
    ...
    FirebaseJsonData result;
    result.stringValue = data.stringData();
    // FirebaseJsonData::to<T>() below actually reads from result.stringValue
    // regardless of the original type, since Firebase_ESP_Client streams
    // typed leaves via the specific data.xxxData() accessors - stash into
    // stringValue and let to<float>()/to<bool>() parse it back out.
    switch (data.dataTypeEnum()) {
      case fb_esp_rtdb_data_type_integer: result.stringValue = String(data.intData()); break;
      case fb_esp_rtdb_data_type_float:
      case fb_esp_rtdb_data_type_double:  result.stringValue = String(data.doubleData(), 6); break;
      case fb_esp_rtdb_data_type_boolean: result.stringValue = data.boolData() ? "1" : "0"; break;
      case fb_esp_rtdb_data_type_string:  result.stringValue = data.stringData(); break;
      default: break;
    }
    applySettingFromFirebase(jkBmsDevices[0], key, result);
  }
...
void firebaseSettingsStreamTimeoutCallback(bool timeout) {
  if (timeout) {
    DEBUG_PRINTLN("Firebase settings stream timeout, resuming...");
  }
  if (!fbdoStream.httpConnected()) {
    DEBUG_PRINTF("Firebase settings stream error: %s\n", fbdoStream.errorReason().c_str());
  }
}
...
void setup() {
  ...
  // FIX: NEW - listen for dashboard-written settings and relay them to the
  // real BMS over BLE. Without this, the whole write pipeline (even once
  // fixed) has nothing driving it - the dashboard's OK button would keep
  // writing to Firebase into the void.
  String settingsPath = "/JK_BMS_Hubs/" + getEspID() + "/" + String(jkBmsDevices[0].targetMAC.c_str()) + "/settings";
  DEBUG_PRINTF("Subscribing to settings stream at: %s\n", settingsPath.c_str());
  if (!Firebase.RTDB.beginStream(&fbdoStream, settingsPath.c_str())) {
    DEBUG_PRINTF("Failed to begin settings stream: %s\n", fbdoStream.errorReason().c_str());
  }
  Firebase.RTDB.setStreamCallback(&fbdoStream, firebaseSettingsStreamCallback, firebaseSettingsStreamTimeoutCallback);
...
void loop() {
  ...
  for (int i = 0; i < bmsDeviceCount; i++) {
    ...
    if (jkBmsDevices[i].uploadStatusPending) {
      ...
      // All parameter writes now come from the Firebase settings stream
      // (firebaseSettingsStreamCallback -> applySettingFromFirebase), i.e.
